...
//  Wurstfinger
//
//  Resolver that delegates to a fallback mode (e.g. numeric) when the
...
///
/// Inspired by Thumb-Key's "ghost keys" — letters that surface their hidden
/// numeric-mode binding on the same key without a mode switch. The resolver
/// is constructed with the *resolved* fallback mode (rather than just a name)
...
struct GhostKeyResolver: GestureResolver {
    ...
    func resolve(keyId: String, gesture: GestureType, in mode: KeyboardMode) -> KeyBinding? {
        ...
        // digit. This lets the numeric mode expose its digits to a hold on a
        // letter key without mirroring every `.tap` as an identical
